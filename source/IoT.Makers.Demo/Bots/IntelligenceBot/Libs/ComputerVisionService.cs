...
namespace IntelligenceBot
{
    public class ComputerVisionService
    {
        ...
        {
            var x = await UploadAndRecognizeImage(imageFilePath, GetSupportedLanguages().Where(y => y.ShortCode == "en").SingleOrDefault().ShortCode);
        ...
        public async Task<string> RecognizeImageFromUrl(string UrlImage)
        {
            var x = await AnalyzeUrl(UrlImage);
        ...
        public async Task<string> RecognizeImage(string imageFilePath)
        {
            var x = await UploadAndAnalyzeImage(imageFilePath);
            var result = LogAnalysisResult(x);
            return result;
        }
        ...
        private async Task<AnalysisResult> UploadAndAnalyzeImage(string filePath)
        {
        ...
        private async Task<OcrResults> UploadAndRecognizeImage(string filePath, string language)
        {
            var stream = new FileStream(filePath, FileMode.Open, FileAccess.Read);
