...
var mqtt_receiver_1 = require("./mqtt_receiver");
var dbg = require("debug");
var debug = dbg('mqtt-common');
...
var MqttBase = (function () {
    ...
    MqttBase.prototype.connect = function (config, done) {
        ...
        if (done) {
            var self_1 = this;
            var errCallback_1 = function (error) {
                var err = error || new azure_iot_common_1.errors.NotConnectedError('Unable to establish a connection');
                self_1.client.removeListener('close', errCallback_1);
                self_1.client.removeListener('offline', errCallback_1);
                self_1.client.removeListener('disconnect', errCallback_1);
                self_1.client.removeListener('error', errCallback_1);
                done(err);
            };
            ...
            self_1.client.on('error', errCallback_1);
            self_1.client.on('close', errCallback_1);
            self_1.client.on('offline', errCallback_1);
            self_1.client.on('disconnect', errCallback_1);
            self_1.client.on('connect', function (connack) {
                debug('Device is connected');
                debug('CONNACK: ' + JSON.stringify(connack));
                self_1.client.removeListener('close', errCallback_1);
                self_1.client.removeListener('offline', errCallback_1);
                self_1.client.removeListener('disconnect', errCallback_1);
                self_1.client.on('close', function () {
                    debug('Device connection to the server has been closed');
                    self_1._receiver = null;
                });
    ...
    MqttBase.prototype.getReceiver = function (done) {
        if (!this._receiver) {
            this._receiver = new mqtt_receiver_1.MqttReceiver(this.client, this._topicMessageSubscribe);
        }
