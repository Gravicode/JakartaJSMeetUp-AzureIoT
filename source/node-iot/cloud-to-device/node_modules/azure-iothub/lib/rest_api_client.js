...
var RestApiClient = (function () {
    function RestApiClient(config, httpRequestBuilder) {
        ...
        if (!config.sharedAccessSignature)
            throw new azure_iot_common_1.errors.ArgumentError('config.sharedAccessSignature cannot be \'' + config.sharedAccessSignature + '\'');
        this._config = config;
        ...
        this._http = httpRequestBuilder || new azure_iot_http_base_1.Http();
    }
    /**
    ...
    RestApiClient.translateError = function (body, response) {
        ...
        var error;
        var errorContent = azure_iot_http_base_1.Http.parseErrorBody(body);
